...
public sealed class WardSlotsTests
{
    ...
    /// <summary>Puts registration 1 back in the queue as a fixed storefront awaiting review.</summary>
    private static async Task SubmitRegistrationAsync(Fixture f, params string[] evidenceTypes)
    {
        ...
        {
            f.Db.RegistrationEvidences.Add(new RegistrationEvidence
            {
                registration_id = 1,
                evidence_type = evidenceType,
                file_url = $"/api/uploads/evidence/10/{evidenceType}.jpg",
    ...
    [Fact]
    public async Task A_registration_missing_required_evidence_cannot_be_approved_but_can_be_sent_back()
    {
        using var f = await Fixture.Create();
        await SubmitRegistrationAsync(f, "IDENTITY_DOCUMENT");   // BR-07 also wants a licence

        var view = await f.Service.GetAsync(f.Actor, "registrations", 1, default);
        Assert.Contains(view.Blockers, b => b.Contains("Giấy phép kinh doanh"));
        Assert.DoesNotContain("APPROVE", view.Actions);
        Assert.Contains("REQUEST_INFO", view.Actions);

        Assert.Equal(409, (await Assert.ThrowsAsync<WardException>(() =>
            f.Service.DecideAsync(f.Actor, "registrations", 1, new("APPROVE", "Duyệt", "SUBMITTED"), default))).Status);

        var sentBack = await f.Service.DecideAsync(
            f.Actor, "registrations", 1, new("REQUEST_INFO", "Thiếu giấy phép", "SUBMITTED"), default);
        Assert.Equal("MORE_INFORMATION_REQUIRED", sentBack.Status);
    }

    [Fact]
    public async Task Approving_a_registration_records_the_reviewer_and_notifies_the_vendor()
    {
        using var f = await Fixture.Create();
        await SubmitRegistrationAsync(f, "IDENTITY_DOCUMENT", "BUSINESS_LICENSE");

        var result = await f.Service.DecideAsync(
            f.Actor, "registrations", 1, new("APPROVE", "Hồ sơ hợp lệ", "SUBMITTED"), default);

        Assert.Equal("APPROVED", result.Status);
        var registration = await f.Db.BusinessRegistrations.FindAsync(1L);
        Assert.Equal(f.Actor.UserId, registration!.reviewed_by);          // BR-46
        Assert.NotNull(registration.reviewed_at);
        Assert.Equal("Hồ sơ hợp lệ", registration.review_decision_reason);
        Assert.Single(await f.Db.AuditLogs.ToListAsync());                // SEC-10
        Assert.Single(await f.Db.Notifications.ToListAsync());
    }

    [Fact]
    public async Task A_registration_can_be_claimed_for_review_before_a_decision()
    {
        using var f = await Fixture.Create();
        await SubmitRegistrationAsync(f, "IDENTITY_DOCUMENT", "BUSINESS_LICENSE");

        var claimed = await f.Service.DecideAsync(
            f.Actor, "registrations", 1, new("REVIEW", "Đang xem", "SUBMITTED"), default);

        Assert.Equal("UNDER_REVIEW", claimed.Status);
        Assert.DoesNotContain("REVIEW", claimed.Actions);
        Assert.Contains("APPROVE", claimed.Actions);
    }

    [Fact]
    public async Task Registration_evidence_is_listed_for_the_reviewing_officer_only()
    {
        using var f = await Fixture.Create();
        await SubmitRegistrationAsync(f, "IDENTITY_DOCUMENT", "BUSINESS_LICENSE");

        var view = await f.Service.GetAsync(f.Actor, "registrations", 1, default);
        Assert.Equal(2, view.Documents!.Length);

        // PRI-02: another ward cannot even see that the case exists.
        Assert.Equal(404, (await Assert.ThrowsAsync<WardException>(() =>
            f.Service.GetAsync(new WardActor(2, 2, "Other"), "registrations", 1, default))).Status);
    }

    [Fact]
    public async Task Fast_tracked_registrations_come_first_in_the_queue()
    {
        using var f = await Fixture.Create();
        await SubmitRegistrationAsync(f, "IDENTITY_DOCUMENT", "BUSINESS_LICENSE");
        var second = (await f.Db.BusinessRegistrations.FindAsync(2L))!;
        second.registration_status = "SUBMITTED";
        (await f.Db.BusinessRegistrations.FindAsync(1L))!.fast_track_flag = true;   // REG-06
        await f.Db.SaveChangesAsync();

        var page = await f.Service.ListAsync(f.Actor, "registrations", 1, default);

        Assert.Equal("1", page.Items[0].Id);
        Assert.True(page.Items[0].FastTrack);
    }
